function Result = F_stochastic_bidding_2(Para,agent,Num)
...
% 价格接受者，提交price-quantity pairs
    ...
    for w = 1:Num.S
        ...
        Var.scene(w).discharing = sdpvar(Num.T,ESSno);
    ...
    obj = 0;
    for w = 1:Num.S
        obj_scenario(w) =  sum(Para.scenario(w).price * (Var.scene(w).discharing - Var.scene(w).charging)) + ...
            sum(Para.scenario(w).price * 0.8 * Var.scene(w).RTdischaring) -  sum(Para.scenario(w).price * 1.2 * Var.scene(w).RTcharing) - ...
    ...
    for w = 1:Num.S
        ...
        Result.scene(w).discharing = value(Var.scene(w).discharing);
