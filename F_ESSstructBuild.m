...
for i = 1:Num.ESS
    ...
    if isempty(find(strcmp(Name_ESS,'costblock')))
         storage(i).discost = Rawdata_ESS(i,10);
         storage(i).chacost = Rawdata_ESS(i,11);
    else
        tempi = find(strcmp(Name_ESS,'costblock'));
        ...
        storage(i).Pdismaxb = ones(1,Num.ESScostblock) * storage(i).Pdismax/Num.ESScostblock;
