...
% 对比ESS的利润(所有ESS)
for w = 1:Num.S
    welfareESS0(w,:) = Resultorigin(w).cal.welfare.ESS;
...
[ESSschedule1(w).schedule; ESSschedule2(w).schedule; ESSschedule3(w).schedule];


for w = 1:Num.S
    ESSschedule1(w).SOC = Resultafter1(w).SOC(bidagent,:);
    ESSschedule2(w).SOC = Resultafter2(w).SOC(bidagent,:);
    ESSschedule3(w).SOC = Resultwith(w).SOC(bidagent,:);
end 
[ESSschedule1(w).SOC; ESSschedule2(w).SOC; ESSschedule3(w).SOC]
