...
%             Cons = [Cons, Var.chargeprice(t,i) <= Var.dischargeprice(t,i)];
    ...
    obj = 0;
    for w = 1:Num.S
        obj_scenario(w) =  sum(Para.scenario(w).price * (Var.scene(w).discharging - Var.scene(w).charging)) + ...
            sum(Para.scenario(w).price * 0.97 * Var.scene(w).RTdischarging) -  sum(Para.scenario(w).price * 1.03 * Var.scene(w).RTcharging) - ...
